...
<div class="iwe-wrap wrap">
    <h2 class="in-title"><?php echo __('Members', 'inwavethemes'); ?>
        <!--<a class="bt-button add-new-h2" href ="<?php echo admin_url('edit.php?post_type=iwevent&page=volunteer/addnew'); ?>"><?php echo __("Add New"); ?></a>-->
    ...
    <form id="payment-form" action="<?php echo admin_url(); ?>admin-post.php" method="post">
        <input type="hidden" name="action" value="inFundingDeleteVolunteers"/>
        <table class="wp-list-table widefat fixed posts">
            <thead>
                <tr>
                    ...
                    foreach ($member_list['fields'] as $field_header) {
                        echo '<th>' . __($field_header['label'], 'inwavethemes') . '</th>';
                    }
            ...
            <tbody id="the-list">
                ...
                if (!empty($member_list['data']) && $member_list['data'][0]['id']) {
                    foreach ($member_list['data'] as $member_info) {
                        ...
                        <tr>
                            <th scope="row" class="check-column">
                                <input id="cb-select-1" type="checkbox" name="fields[]" value="<?php echo esc_attr($member_id); ?>"/>
                    ...
                    ?>
                    <td>
                        <a href="<?php echo admin_url('edit.php?post_type=infunding&page=volunteer/edit&id=' . $member_id); ?>" title="<?php echo __('Edit this member', 'inwavethemes'); ?>"><?php echo __('Edit', 'inwavethemes'); ?></a> |
                        <a class="submitdelete" title="<?php echo __('Delete this member', 'inwavethemes'); ?>" href="<?php echo admin_url("admin-post.php?action=inFundingDeleteVolunteer&id=" . $member_id); ?>"><?php echo __('Delete', 'inwavethemes'); ?></a>
                    </td>
                    </tr>
                    <?php
                }
